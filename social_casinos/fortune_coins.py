...
class FortuneCoins(BaseCasino):
    ...
    def login(self):
        ...
        self.accept_tos()
        self.dismiss_bonus_popup()

    def accept_tos(self):
        tos_wait = WebDriverWait(self.driver, self.TOS_WAIT)
        try:
            tos_wait.until(EC.url_contains('fortunewins.com/lobby'))
            scroll_area = tos_wait.until(EC.presence_of_element_located(self.TOS_SCROLL))
            self.driver.execute_script('arguments[0].scrollTop = arguments[0].scrollHeight', scroll_area)
            checkbox = tos_wait.until(EC.presence_of_element_located(self.TOS_CHECKBOX))
            self.driver.execute_script('arguments[0].click()', checkbox)
            confirm = tos_wait.until(EC.presence_of_element_located(self.TOS_CONFIRM))
            self.driver.execute_script('arguments[0].click()', confirm)
        except TimeoutException:
            pass

    def dismiss_bonus_popup(self):
        popup_wait = WebDriverWait(self.driver, self.BONUS_POPUP_WAIT)
        try:
            btn = popup_wait.until(EC.element_to_be_clickable(self.BONUS_POPUP_BTN))
